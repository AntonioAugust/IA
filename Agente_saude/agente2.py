...
data.drop(data.columns[[0]], axis=1, inplace=True)#remove coluna paciente
...
def calcular_entropia(coluna_y):
    ...
    #print("Total de amostras de cada valor:", contagens)
...
def predict_usuario(arvore, atributos):
    ...
    classe_predita = arvore.predict_linha(arvore.root, linha)
...
class decision_tree():
    ...
    def build_tree(self, X, Y):
        ...
        self.ganhos_de_informacao = ganhos
    ...
    def get_ganhos(self):
        return self.ganhos_de_informacao


    def predict_linha(self, node, linha):
        if node.is_leaf():
            return node.classe
        
        valor = linha[node.atributo]
        if valor in node.filhos:
            return self.predict_linha(node.filhos[valor], linha)
        else:
...
print(f"Atributo da raiz:{arvore.root.atributo}, Filhos da raiz:{arvore.root.filhos.keys()}")
